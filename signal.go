...
func (ctx *signalContext) Listen() {
	...
	log.Printf("Exec `kill -INT %d` will graceful exit", os.Getpid())
	log.Printf("Exec `kill -TERM %d` will graceful restart", os.Getpid())
	...
	if ctx.IsHTTP {
		...
		log.Println("Stopping console server")
		switch s {
		case syscall.SIGTERM:
			...
			if err != nil {
				log.Printf("failed to forkexec: %v", err)
			}
...
func (ctx *signalContext) doShutdownDone() {
	...
	log.Println("doShutdownDone start.")
	defer log.Println("doShutdownDone done.")
	...
	case <-time.After(time.Duration(timeout) * time.Second):
		log.Printf("doShutdownDone %ds timeout", timeout)
		close(ctx.done)
	case <-ctx.done:
	}
}
...
func (ctx *signalContext) waitDone() {
	//context包来取消，以通知业务方
	log.Println("signal ctx cancel")
	ctx.Cancel()
	//等待业务方完成退出
	log.Println("signal ctx waitgroup wait done start")
	ctx.WgWait()
	//表示全部完成
	log.Println("signal ctx waitgroup wait done end")
	close(ctx.done)
}
